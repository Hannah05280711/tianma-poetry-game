...
/** 检查某章节（每3关一章）的3关是否全部通关 */
export async function isChapterComplete(sessionKey: string, stageNumber: number): Promise<boolean> {
  const db = await getDb();
  if (!db) return false;
  // 计算章节的起始关卡编号（1-3为第一章，4-6为第二章...）
  const chapterIndex = Math.ceil(stageNumber / 3);
  const chapterStart = (chapterIndex - 1) * 3 + 1;
  const chapterEnd = chapterIndex * 3;
  // 获取该章节3关的stage IDs
  const stages = await db.select({ id: v2Stages.id, stageNumber: v2Stages.stageNumber })
    .from(v2Stages)
    .where(and(
      gte(v2Stages.stageNumber, chapterStart),
      lte(v2Stages.stageNumber, chapterEnd)
    ));
  if (stages.length < 3) return false;
  // 检查3关是否全部completed
  const stageIds = stages.map(s => s.id);
  const progresses = await db.select().from(userStageProgress).where(
    and(
      eq(userStageProgress.sessionKey, sessionKey),
      inArray(userStageProgress.stageId, stageIds),
      eq(userStageProgress.status, "completed")
    )
  );
  return progresses.length >= 3;
}
